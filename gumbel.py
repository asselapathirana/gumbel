...
ser_=pd.read_csv('data.SHIA.csv')
ser=ser_[:10]
...
#qi=(i=a)/(N+1-2a) - so Tr=(N+1=2a)/(i-a)
xtl=[x*MAXFANNFREQ for x in xticklabels]
xticks=[-np.log(-np.log(1-1/x)) for x in xtl]
#ser_lin=ser.loc[1/ser['qi'] > 2]
#ser_lin=ser_lin.loc[1/ser['qi']<100]
...
textstr = r'$y={:4f}(-ln(-ln(1-1/({}Tr))+{:4f} $'.format(m,MAXFANNFREQ, c)
